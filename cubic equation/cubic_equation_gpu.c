...
int main() {
    ...
    const char * kernel_code = load_kernel_source("kernels/cubic_equation.cl", & err);
    if (err != 0) {
    ...
    if (err != CL_SUCCESS) {
		size_t real_size;
		err = clGetProgramBuildInfo(program,device,CL_PROGRAM_BUILD_LOG,0,NULL,&real_size);
		char* build_log = (char*)malloc(sizeof(char)*(real_size + 1));
		err = clGetProgramBuildInfo(program,device,CL_PROGRAM_BUILD_LOG,real_size+1,build_log,&real_size);
		printf("\n\nRealsize: %d\n", real_size);
		printf("\n\nBuild log: %s\n", build_log);
        printf("Error building program: %d\n", err);
        return -1;
    }
	//Using malloc, instead of regular array defining.
    while (n != 100000000) {
